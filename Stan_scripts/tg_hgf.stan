...
model {
  ...
  for (i in 1:N) {
    ...
    // print("hm, c[i] is ", c[i]);
    mu_good2_prev = mu0[i];
    pi_good2_prev = 1/2.0;// 1.0/sa0_2[i];
    
    // mu_bad1 = mu0_1[i];
    // pi_bad1 = 1.0/sa0_1[i];
    mu_bad2_prev = mu0[i];
    pi_bad2_prev = 1/2.0;// 1.0/sa0_2[i];
    ...
    for (t in 1:Tsubj[i])  {
      ...
      if(trustee[i,t]== 1) { 
        ...
        mu  =mu_good2; //((mu_good1^gam[i]))/((mu_good1^gam[i]) + (1-mu_good1)^gam[i]);
        ...
        // model
        // mu = mu_bad1;
        mu  =mu_bad2; //((mu_bad1^gam[i]))/((mu_bad1^gam[i]) + (1-mu_bad1)^gam[i]);
